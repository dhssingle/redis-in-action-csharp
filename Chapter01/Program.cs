...
const int OneWeekInSeconds = 7 * 86400;
const int VoteScore = 432;
const int ArticlePerPage = 25;

var redis = await ConnectionMultiplexer.ConnectAsync("localhost");
var db = redis.GetDatabase();

async Task ArticleVoteAsync(IDatabase db, string user, string article)
{
    // 计算投票截止时间
    var cutoff = GetUnixTime() - OneWeekInSeconds;
    // 检查是否可以对文章进行投票
    ...
    if (await db.SetAddAsync("voted:" + articleId, user))
    {
        ...
        await db.SortedSetIncrementAsync("score:", article, VoteScore);
    }
}

async Task PostArticleAsync(IDatabase db, string user, string title, string link)
{
    ...
    await db.KeyExpireAsync(votedKey, TimeSpan.FromSeconds(OneWeekInSeconds));
    ...
    await db.SortedSetAddAsync("score:", articleKey, now + VoteScore);
    await db.SortedSetAddAsync("time:", articleKey, now);
}

async Task<List<Dictionary<RedisValue, RedisValue>>> GetArticleAsync(IDatabase db, int page, string order = "order:")
{
    var start = (page - 1) * ArticlePerPage;
    var end = start + ArticlePerPage - 1;
...
async Task AddRemoveGroupsAsync(IDatabase db, string articleId, string[] toAdd, string[] toRemove)
{
...
async Task<List<Dictionary<RedisValue, RedisValue>>> GetGroupArticlesAsync(IDatabase db, string group, int page, string order = "score:")
{
    var key = order + group;
...
static long GetUnixTime() => DateTimeOffset.Now.ToUnixTimeSeconds();